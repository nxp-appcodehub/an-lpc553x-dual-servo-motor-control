...
typedef struct
{
    SM_APP_STATE_FCN_T const*	psStateFast;			/* State functions fast*/
...
/* State machine fast function table */
extern const PFCN_VOID_PSM g_SM_STATE_TABLE_FAST[4];

/* State machine slow function table */
extern const PFCN_VOID_PSM g_SM_STATE_TABLE_SLOW[4];

/* State Machine fast application state machine function */
extern inline void SM_StateMachineFast(SM_APP_CTRL_T *sAppCtrl)
{
	g_SM_STATE_TABLE_FAST[sAppCtrl->eState](sAppCtrl);
}

/* State Machine slow application state machine function */
extern inline void SM_StateMachineSlow(SM_APP_CTRL_T *sAppCtrl)
{
